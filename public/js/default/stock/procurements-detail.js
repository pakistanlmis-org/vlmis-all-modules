$(function () {
    ...
    $("#print_shipment_detail").click(function () {
        var id = $('#hdn_master_id').val();
        window.open(appName + '/stock/print-shipment-detail?id=' + id, '_blank', 'scrollbars=1,width=860,height=595');
    });

    if ($("#item_id").val() != '') {
        getProductsByStakeholder($("#item_id").val());
        getQuantityByShipment($("#hdn_shipment_id").val());
    ...
    $("#item_id").change(function () {
        ...
        getProductPairByProduct(item_id);
        ...
        $.ajax({
            ...
            dataType: 'html',
            success: function (data) {
                $('#cold_chain').html('<option value="" selected="selected">Select</option>');
                if (data == 1) {
...
//                }
    ...
    function getProductsByStakeholder(item_id) {
    ...
    function getQuantityByShipment(shipment_id) {

        $.ajax({
            type: "POST",
            url: appName + "/stock/ajax-get-quantity-by-shipment",
            data: {
            ...
            buttons: (self.data('type') != 'confirm') ? false : [
                {
                    ...
                    onClick: function ($notyfy) {
                        ...
                        var shipment_id = $('#hdn_shipment_id').val();
                        $notyfy.close();
                        window.location.href = appName + '/stock/shipment-batch-delete?id=' + id + '&shipment_id=' + shipment_id;
                    }
                },
                {
...
$('#available_quantity').priceFormat({
    prefix: '',
    thousandsSeparator: ',',
    suffix: '',
    centsLimit: 0,
...
$("#print_vaccine_placement").click(function (e) {
    var id = $('#stockid').val();
    window.open(appName + '/stock/print-receive?id=' + id, '_blank', 'scrollbars=1,width=860,height=595');
});

$("#print_vaccine_placement1").click(function (e) {
    var id = $('#stockid1').val();
    window.open(appName + '/stock/print-receive?id=' + id, '_blank', 'scrollbars=1,width=860,height=595');
});
...
$('#vvm_stage').change(function (e) {
    if ($(this).val() == 3) {
        alert("VVM stage is not usable. Are you sure you want to continue?");
    } else if ($(this).val() == 4) {
        alert("VVM stage is not usable. Are you sure you want to continue?");
    }
});
