...
//                            }
//                        });
                        window.location.href = appName + "/stock/delete-issue?id=" + id + "&p=requisition&demandMasterId=" + $('#demand_master_id').val();
                    }
                }, {
...
$("#btn-loading").click(function (e) {
    ...
    var item_qty = $("#item_id option:selected").text();

    var approvedQty = item_qty.split("|")[1];
    var issuedQty = item_qty.split("|")[2];
    approvedQty = approvedQty.replace(/,/g, "");
    issuedQty = issuedQty.replace(/,/g, "");

    if (parseInt(quantity) <= 0) {
        validator.showErrors({
            "quantity": "Quantity should be greater than 0."
        });
    } else if (parseInt(approvedQty) < parseInt(quantity)) {
        validator.showErrors({
            "quantity": "Quantity not available."
        });

    } else if ((parseInt(issuedQty) + parseInt(quantity)) > parseInt(approvedQty)) {
        validator.showErrors({
            "quantity": "Can not issue quantity more than approved quantity."
        });
