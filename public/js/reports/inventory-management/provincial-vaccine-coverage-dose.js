$(function () {
    ...
    $('#prod_sel').change(function () {
        var prodId = $(this).val();
        showProdDoses(prodId);
    });
});

function showProdDoses(prodId) {
    $.ajax({
        type: "POST",
        url: appName + "/reports/inventory-management/ajax-product-dose",
        data: {item_id: prodId},
        dataType: 'html',
        success: function (data) {
            $('#prod_dose').html(data);
            $('#prod_dose').val($('#dose_sel').val())
        }
    });
}
