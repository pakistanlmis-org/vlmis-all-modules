$(function() {
    ...
    window.location.href = appName + '/iadmin/manage-products/item-units';
});
    $('#records').change(function() {
        var counter = $(this).val();
        document.location.href = appName + '/iadmin/manage-products/item-units/?counter=' + counter;
    });
    ...
    $("#add-stakeholder").validate({
        rules: {
            item_unit_name: {
                required: true,
                remote: {
                    url: appName + "/iadmin/manage-products/check-item-unit",
                    type: "post",
                    data: {
                        item_unit_name_hidden: function () {
                            return '';
                        }
                    }
        ...
        messages: {
            item_unit_name: {
                required: "Please enter item unit.",
                remote: "Item unit already exists."
            }
        }
    });
        ...
        rules: {
            item_unit_name: {
                ...
                remote: {
                    ...
                    data: {
                        item_unit_name_hidden: function () {
                            return $("#item_unit_name_hidden").val();
                        }
                    }
    ...
    $(".update-asset-sub-type").click(function() {
        $.ajax({
            type: "POST",
            url: appName + "/iadmin/manage-products/ajax-item-units-edit",
            data: {item_unit_id: $(this).attr('id')},
            dataType: 'html',
