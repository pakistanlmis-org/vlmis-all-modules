...
class clsItemUnits {
    ...
    function AddItemUnit() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error AddItemUnit");
...
//query result
        $rsSql = mysqli_query($strSql) or die("Error EditItemUnit");
    ...
    function DeleteItemUnit() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error DeleteItemUnit");
        if (mysql_affected_rows()) {
            return TRUE;
        } else {
            return FALSE;
    ...
    function GetAllItemUnits() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error: GetAllItemUnits");
    ...
    function GetItemUnitById() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error: GetItemUnitById");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetUnitByItemId($item_id) {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error: GetUnitByItemId");
        if (mysql_num_rows($rsSql) > 0) {
