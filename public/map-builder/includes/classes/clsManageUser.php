...
class clsUser {
    ...
    function AddUser() {
		...
		VALUES('".(!empty($this->m_user_level)?$this->m_user_level:'')."',NOW()," . ($this->GetMaxUserId() + 1) . "," . $this->m_stkid . "," . $this->m_prov_id . ",'" . $this->m_usrlogin_id . "','" . $this->m_full_name . "','" . $this->m_sysusr_pwd . "','Active','" . $this->m_email_id . "','" . $this->m_phone_no . "','" . $this->m_fax_no . "','" . $this->m_address . "','" . $this->m_sysusr_dept . "','" . $this->m_sysusr_deg . "','" . $this->m_sysusr_type . "','" . $this->m_sysusr_photo . "','" . $hash . "')";
           //echo $strSql;
           //exit;
            //query result
            $rsSql = mysqli_query($strSql) or die("Error AddUser");
    ...
    function EditUser() {
        ...
        $sysuser_level = ",user_level = '" . $this->m_user_level . "'";
        if ($this->m_user_level != '') {
            $strSql .= $sysuser_level;
        }
        ...
        //echo $strSql;exit;
        //query result
        $rsSql = mysqli_query($strSql) or die("Error 'EditUser' Query: $strSql");
    ...
    function DeleteUser() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error DeleteUser");
        if (mysql_affected_rows()) {
            return TRUE;
        } else {
            return FALSE;
...
//query result
        $rsSql = mysqli_query($strSql) or die(mysql_error());
...
//query result
    ...
    function GetUserByprovtitle() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error UserByprovtitle");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetUserByid() {
        ...
        //query result
        ///echo $strSql;exit;
        $rsSql = mysqli_query($strSql) or die("Error GetUserByid");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
     ...
     * @return boolean
     */
    function GetUserByUsernameOnly() {
        //select query
        //gets
        //User ID,
        //stakeholder id,
...
//echo  $strSql;exit;        
//query result
        $rsSql = mysqli_query($strSql) or die("Error GetUserByUserID");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
...
//query result
        $rsSql = mysqli_query($strSql) or die("Error GetAdminByAdminID");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetMaxUserId() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("GetMaxUserId");
