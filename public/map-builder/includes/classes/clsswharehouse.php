...
//npkid
    ...
    function Addwarehouse() {
        ...
        if ($this->is_allowed_im == '') {
            $this->is_allowed_im = 0;
        }
        if (empty($this->hf_type_id) || $this->hf_type_id == '') {
            $this->hf_type_id = 0;
        }
        ...
        //echo $strSql;exit;
        $rsSql = mysqli_query($strSql) or die("Error Addwarehouse");
    ...
    function Editwarehouse() {
        ...
        if ($this->is_allowed_im != '') {
            ...
            $strSql .= ',is_allowed_im=0';
        }
        $strSql .= " WHERE wh_id=" . $this->m_npkId;
        //echo $strSql;
        $rsSql = mysqli_query($strSql) or die("Error Editwarehouse");
    ...
    function Deletewarehouse() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error Deletewarehousehhhh");
        if (mysql_affected_rows()) {
            return TRUE;
        } else {
            return FALSE;
    ...
    function allowIM() {
        ...
        $resQry = mysqli_query($getStatusQry);
        ...
        ;
        $rsSql = mysqli_query($strSql) or die("Error allow_im_update");
        if (mysql_affected_rows()) {
            return TRUE;
        } else {
            return FALSE;
    ...
    function GetAllWarehouses() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetAllwarehouses");
    ...
    function GetAllUCCenters($id) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetAllUCCenters");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetUserWarehouses() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetUserWarehouses");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetUserIssueToWH($wh_id) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetUserIssueToWH");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetUserReceiveFromWH($wh_id) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetUserReceiveFromWH");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function getFifthLvlWH($wh_id) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error getFifthLvlWH");
    ...
    function GetLevelDownWarehouses() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetWarehouses");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
...
//select query
        ...
        //query results
        $rsSql = mysqli_query($strSql) or die("Error GetFederalWarehouses: " . mysql_error());
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetProvincialWarehouses($user_prov, $user_stk, $mainstk) {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetProvincialWarehouses");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
...
//select query
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetDivsionalWarehousesofProvince");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetDistrictWarehousesofProvince($user_prov, $user_stk, $mainstk) {
        ...
        $strSql = " SELECT
					...
					ORDER BY
						tbl_warehouse.wh_name ASC";
        //query result
        $rsSql = mysqli_query($strSql) or die($strSql);
...
//select query
    ...
    function GetWarehousesByLevel($from, $to) {
        ...
        $strSql = "SELECT
			...
			stakeholder.lvl <= " . $to;
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetWarehousesByLevel");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetWarehouseLevelById($id) {
        ...
        //query results
        $rsSql = mysqli_query($strSql) or die("Error GetWarehouseLevelById");
        if (mysql_num_rows($rsSql) > 0) {
    ...
    function GetWarehouseById() {
        ...
        $strSql = "SELECT
                    ...
                    province.PkLocID AS prov_id,
                    province.LocName AS province,
                    (select stkofc.lvl from stakeholder as stkofc where stkofc.stkid = tbl_warehouse.stkofficeid  ) as stkofc_level
				FROM
					tbl_warehouse
				Left Join stakeholder ON tbl_warehouse.stkid = stakeholder.stkid
				Left Join tbl_locations AS district ON tbl_warehouse.locid = district.PkLocID
				...
				WHERE
					wh_id=" . $this->m_npkId;
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetwarehouseById");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetWarehouseNameById() {
        ...
        $strSql = "SELECT
				...
				WHERE
					wh_id = " . $this->m_npkId;
        //query result
        $rsSql = mysqli_query($strSql) or die("GetWarehouseNameById");
    ...
    function GetStkIDByWHId() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("GetStkIDByWHId");
        if ($rsSql != FALSE && mysql_num_rows($rsSql) > 0) {
            while ($row = mysql_fetch_array($rsSql)) {
                $whName = $row['Stkid'];
            }
    ...
    function IsDivWHofProvWH($div_id, $prov_id) {
        ...
        $strSql = " SELECT
					...
					AND stakeholder.lvl = 2";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error: IsDivWHofProvWH -> clsswharehouse.php");
        if ($rsSql != FALSE && mysql_num_rows($rsSql) > 0) {
            return 1;
        } else {
            return 0;
    ...
    function IsSubWHofDistWH($SubWH_id, $Dist_id) {
        ...
        $strSql = " SELECT
					...
					AND stakeholder.lvl = 4";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error: IsSubWHofDistWH -> clsswharehouse.php");
        if ($rsSql != FALSE && mysql_num_rows($rsSql) > 0) {
            return 1;
        } else {
            return 0;
    ...
    function GetWarehouseBylocByStakeholderOffice() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetWarehouseBylocByStakeholderOfficee");
    ...
    function GetWarehouseBylocByStakeholder() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetWarehouseBylocByStakeholder");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetAllWarehouses0() {
        ...
        $strSql = "SELECT
            ...
            ORDER BY
                    ...
                    officeName";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetAllwarehouses0");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
...
//query result
        $rsSql = mysqli_query($strSql) or die("Error GetWHTypes");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetProvincialLocations() {
        //select query
        //gets
        //warehouse id
        //warehouse name
