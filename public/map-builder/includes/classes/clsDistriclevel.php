...
class clstbl_dist_levels {
    ...
    function Addtbl_dist_levels() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error Addtbl_dist_levels");
    ...
    function Edittbl_dist_levels() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error Edittbl_dist_levels");
    ...
    function Deletetbl_dist_levels() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error Deletetbl_dist_levels");
        if (mysql_affected_rows()) {
            return TRUE;
        } else {
            return FALSE;
    ...
    function GetAlllevelsById() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetAlllevelsById");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetLowerLevels() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetLevelsById");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
