...
class clsstakeholderitem {
    ...
    function Addstakeholderitem() {
        ...
        while ($stk_n != 0) {
            ...
            //query result
            $rsSql = mysqli_query($strSql) or die("Error Addstakeholderitem");
            $no++;
            $stk_n--;
        }
 ...
 */
    function UpdateStakeholderItem() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error UpdateStakeholderItem");
        if (mysql_affected_rows()) {
            return TRUE;
        } else {
            return FALSE;
    ...
    function Editstkholderitem() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error Deletestakeholderitem");
        if ($this->m_stkid == '') {
            $this->m_stkid = 0;
        }
        if ($this->m_stk_item == '') {
    ...
    function Editstakeholderitem() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error stakeholderitem");
        if (mysql_affected_rows()) {
            return $rsSql;
        } else {
            return FALSE;
...
//select query
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetAllstakeholderitem");
...
//query result
        $rsSql = mysqli_query($strSql) or die("Error GetstakeholderitemById");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
...
//query result
    ...
    function GetstakeholderItemsById() {
        ...
        $strSql = "
				...
				Where stakeholder_item.stkid=" . $this->m_npkId;
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetstakeholderItemsById");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
