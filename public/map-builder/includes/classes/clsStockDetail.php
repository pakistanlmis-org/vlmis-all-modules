...
class clsStockDetail {
    ...
    public
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error: find_by_stock_id: " . mysql_error());
    ...
    public
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error find_by_detail_id" . $strSql);
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    public function StockReceived($id) {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error StockReceived");
    ...
    public function updateDetail($oldid, $new_batch_id, $stk_detail_id) {
        mysqli_query("Update " . static::$table_name . " set BatchID=$new_batch_id where PkDetailID = ".$stk_detail_id." ") or die("Error updateDetail");
        if (mysql_affected_rows() > 0) {
            $row = mysql_fetch_array(mysqli_query("SELECT
            ...
            mysqli_query("Update stock_batch set Qty=" . $row['quantity'] . " where batch_id = $new_batch_id");
            return true;
        } else {
            return FALSE;
        }
    ...
    function adjustQty($id, $qty) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error adjustQty");
    ...
    public function getQtyById($id) {
        $strSql = "SELECT
                ...
                where PkDetailID=$id";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error getQtyById");
    ...
    public function getBatchQtyById($id) {
       ...
       //query result
        $rsSql = mysqli_query($strSql) or die("Error getBatchQtyById");
        if (!empty($rsSql) && mysql_num_rows($rsSql) > 0) {
            $row = mysql_fetch_object($rsSql);
            return $row->Qty;
        } else {
    ...
    public function GetBatchDetail($id) {
        $strSql = "SELECT
			...
			tbl_stock_detail.PkDetailID = $id";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetBatchDetail");
        if (mysql_num_rows($rsSql) > 0) {
