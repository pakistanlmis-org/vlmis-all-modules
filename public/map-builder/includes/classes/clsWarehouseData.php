...
class clsWarehouseData {
    ...
    function getLastReport($wh_id, $RptDate, $itemId) {
        ...
        $rsSql = mysqli_query($strSql) or die("getLastReport");
    ...
    function getMonthYearByWHID($wh_id) {
        ...
        $rsSql = mysqli_query($strSql) or die("getMonthYearByWHID");
        if ($rsSql != FALSE && mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function closingBalanceOfMonth($month, $year, $wh_id, $item_id) {
		...
		tbl_wh_data.wh_id = $wh_id";
        $rsSql = mysqli_query($strSql) or die("openingBalance");
        if ($rsSql != FALSE && mysql_num_rows($rsSql) > 0) {
            $row = mysql_fetch_object($rsSql);
            return $row->cbv;
        } else {
    ...
    function getDataByWhId() {
        ...
        $rsSql = mysqli_query($strSql) or die("getDataByWhId");
        if ($rsSql != FALSE && mysql_num_rows($rsSql) > 0) {
            return TRUE;
        } else {
            return FALSE;
					...
					tbl_wh_data.item_id = '" . $productrec_id . "'";
                            ...
                            if ($rsSql != FALSE && mysql_num_rows($rsSql) > 0) {
                                ...
                                if (!empty($report_data['w_id'])) {
                                    ...
                                    mysqli_query($updateData) or die("update to wh_data");
                                } else {
                                    ...
                                    mysqli_query($saveData) or die("save to wh_data");
                                }
                            }
                        }
                        break;
