...
//echo 'upd:::'.$sql;exit;
    ...
    function stockExists() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error stockExists");
	...
	quantity ASC";
        ...
        //echo $strSql;exit;
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetStockLedger");
        $data = array();
        if (mysql_num_rows($rsSql) > 0) {
    ...
    public function getBatchOBCB($type, $item, $trDate,$funding_source='') {
        ...
        //echo $strSql;exit;
        //query result
        $rsSql = mysqli_query($strSql) or die("Error getBatchOBCB");
        $data = array();
        if (mysql_num_rows($rsSql) > 0) {
            while ($row = mysql_fetch_assoc($rsSql)) {
                $data[] = $row;
    ...
    function GetTempStockIssue($user, $wh_id, $type) {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetTempStockIssue");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetStocksIssueList($userid, $wh_id, $type, $stockid) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetStocksIssueList");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function updateTemp($id) {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error updateTemp");
    ...
    function GetTempStocksIssueList($userid, $wh_id, $type) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetTempStocksIssueList");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetTempStockReceive($user, $wh_id, $type) {
		...
		AND tbl_stock_master.TranTypeID = $type";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetTempStockReceive");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetTempStockRUpdate($PkStockID = 0) {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetTempStockRUpdate");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetTempStocksReceiveList($userid, $wh_id, $type) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error GetTempStocksReceiveList");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetLastInseredTempStocksReceiveList($userid, $wh_id, $type) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error :" . $strSql);
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetStocksReceiveList($userid, $wh_id, $type, $stockid) {
		...
		ORDER BY itminfo_tab.frmindex ASC,
		stock_batch.batch_no ASC";
        $rsSql = mysqli_query($strSql) or die("Error GetStocksReceiveList");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function StockAdjustmentSearch() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error StockAdjustmentSearch");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function StockSearch($type, $wh_id, $groupby = '', $page_type = '') {
        ...
        //echo $strSql;
        $rsSql = mysqli_query($strSql) or trigger_error(mysql_error() . $strSql);
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
...
GROUP BY
    ...
    function getLastID($from, $to, $tr_type) {
        ...
        $rsSql = mysqli_query($strSql) or die("Error getLastID $strSql");
        if (mysql_num_rows($rsSql) > 0) {
    ...
    function GetWHStockByIssueNo() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetWHStockByIssueNo");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function getWhLevelByStockID($stock_id) {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error getWhLevelByStockID");
        if (mysql_num_rows($rsSql) > 0) {
    ...
    function getDateAndItemfromStock($stock_id) {
        ...
        $strSql = "SELECT
         ...
         ";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error getDateAndItemfromStock");
        if (mysql_num_rows($rsSql) > 0) {
            while ($row = mysql_fetch_object($rsSql)) {
                $array[] = array(
                    'date' => $row->TranDate,
    ...
    function getItemDetailfromStock($stock_id) {
        ...
        //query deatil
        $rsSql = mysqli_query($strSql) or die("Error getItemDetailfromStock");
        if (mysql_num_rows($rsSql) > 0) {
            while ($row = mysql_fetch_object($rsSql)) {
                $array[] = array(
                    'date' => $row->TranDate,
...
tbl_stock_detail.IsReceived = 0";
        $rsSql = mysqli_query($strSql) or die("Error getPendingVouchers");
        if (mysql_num_rows($rsSql) > 0) {
            $data = "";
            while ($row = mysql_fetch_assoc($rsSql)) {
