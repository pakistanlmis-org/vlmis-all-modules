...
class clsManageItem {
    ...
    function AddManageItem() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error AddManageItem");
...
//check item status
        ...
        $rsSql = mysqli_query($strSql) or die("Error EditManageItem");
    ...
    function DeleteManageItem() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error DeleteManageItem");
        if (mysql_affected_rows()) {
            return TRUE;
        } else {
            return FALSE;
    ...
    function GetAllManageItem() {
        ...
        $strSql = "SELECT DISTINCT
					...
					ORDER BY
						itminfo_tab.frmindex";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetAllManageItem");
    ...
    function GetManageItemById() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetManageItemById");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
    ...
    function GetAllWHProduct() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetAllWHProduct");
        if (mysql_num_rows($rsSql) > 0) {
    ...
    function GetAllProduct() {
		...
		itminfo_tab.frmindex ASC";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetAllProduct");
        if (mysql_num_rows($rsSql) > 0) {
            while ($row = mysql_fetch_object($rsSql)) {
                $array[] = array(
                    'id' => $row->itm_id,
    ...
    function GetProductName($item_id) {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetProductCat");
        if (mysql_num_rows($rsSql) > 0) {
