...
class clsStakeholderType {
    ...
    function AddStakeholderType() {
        ...
        //query result
        $rsSql = mysqli_query($strSql) or die("Error AddStakeholderType");
    ...
    function EditStakeholderType() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error EditStakeholderType");
    ...
    function DeleteStakeholderType() {
        ...
        $rsSql = mysqli_query($strSql) or die("Error StakeholderType");
        if (mysql_affected_rows()) {
            return TRUE;
        } else {
            return FALSE;
    ...
    function GetAllItStakeholderType() {
        ...
        $strSql = " SELECT
                    ...
                    FROM
                        stakeholder_type";
        //query result
        $rsSql = mysqli_query($strSql) or die("Error GetAllItStakeholderType");
...
//query result
        $rsSql = mysqli_query($strSql) or die("Error GetStakeholderTypeById");
        if (mysql_num_rows($rsSql) > 0) {
            return $rsSql;
        } else {
            return FALSE;
