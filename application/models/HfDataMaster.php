   ...
   /**
    ...
    public function getConsumptionByUc($dist_id) {
        ...
        if ($this->form_values['wastages'] == 2) {
            ...
            $xmlstore .= "</dataset>"; 
            $xmlstore .= "<dataset seriesname='Non Permissible Wastages' color='#FF0000'>";
            foreach ($result as $data) {
                $total = $data[CONS] + $data[wastages];
                $wastageP = Round(($data[wastages] / $total) * 100);
...
//        }
    ...
    public function addMonthlyConsumption2() {
        ...
        $children_aged_12_23_M = $data['children_aged_12_23_M'];
        $two_year_above = $data['2_year_above'];
        $f_im_children = $data['f_im_children'];
        $total_population = $data['total_population'];
        $cba = $data['cba'];
        $fixed_planned_sessions = $data['fixed_planned_sessions'];
        $fixed_actually_held_sessions = $data['fixed_actually_held_sessions'];
        ...
        foreach ($closing_balance_array as $key => $val) {
            if ($val != '' && $val >= 0) {
                ...
                $this->updateHfSessions($data['wh_id'], App_Controller_Functions::dateToDbFormat($data['rpt_date']), $fixed_planned_sessions, $fixed_actually_held_sessions, $outreach_planned_sessions, $outreach_actually_held_sessions,$f_im_children);
                $this->_em->persist($hf_data_master);
                $this->_em->flush();
                $hf_data_master_id = $hf_data_master->getPkId();
                if ($item_category[$key] == 1) {
     ...
     */
    public function updateHfSessions($wh_id, $report_date, $fix_plan_sessions, $fix_actually_held_sessions, $outreach_plan_sessions, $outreach_actually_held_sessions,$f_imm_children) {
        ...
        $hf_sessions->setFullyImmunizedChildren($f_imm_children);
